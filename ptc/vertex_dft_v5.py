"""
PTC v5 — Vertex DFT on Z/(2P_l)Z with NATIVE contextual screening.
...
def vertex_dft_v5(vertex: int, bond_idx: int,
    ...
    # 0 params: S₅, P₂, P₁ from s = 1/2. [v4 C9]
    if is_d:
        nf = d.get('nf', 0)
    ...
    # ════════════════════════════════════════════════════════════
    #  3. ORBITAL COMPETITION (face saturation, DANS le screening)
    # ════════════════════════════════════════════════════════════
    # En PT: quand z bonds occupent le même polygone Z/(2P_l)Z,
    # chaque bond additionnel au-delà de 1 partage le budget spectral.
    # La compétition = (z-1)/z × S₃ = fraction du mode k=0 partagée.
    # C'est le MC1 du v4 mais intégré DANS le screening.
    #
    # Gate: IE >= Ry - S₃ (centres non-hybridisés, N/O/F).
    # Amplitude continue: sin²(π × IE / (2 × Ry)) interpole
    # entre 0 (IE << Ry, hybridisé) et S₃ (IE ≈ Ry, compact).
    ...
    # ════════════════════════════════════════════════════════════
    #  4. FISHER CURVATURE (cross-face coupling, DANS le screening)
    # ════════════════════════════════════════════════════════════
    # P₁⊗P₃ anti-corrélation: pour les vertex compacts (per ≤ 2)
    # avec des partenaires LP-free, les modes P₁ et P₃ interfèrent
    # destructivement → réduit le screening.
    # C'est le C20 du v4 mais sans f_size.
...
def vertex_dft_v5_P2(vertex: int, bond_idx: int,
                      topology: Topology, atom_data: dict) -> float:
    """V5 native vertex DFT on Z/(2P₂)Z (pentagonal face, π + LP).

    Contextual peer floor replaces v4's universal D₇.
...
def vertex_dft_v5_P3(vertex: int, bond_idx: int,
                      topology: Topology, atom_data: dict) -> float:
